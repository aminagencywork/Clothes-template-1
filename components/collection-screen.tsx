...
export function CollectionScreen({ collection }: { collection: Collection }) {
  ...
  const round = "grid size-[calc(var(--u)*92)] place-items-center rounded-full bg-pill/80";
  ...
  return (
    <PhoneFrame>
      <main className="px-[calc(var(--u)*37)] pb-[calc(var(--u)*240)] pt-[calc(var(--u)*60)]">
        ...
        <section className="relative mt-[calc(var(--u)*30)] h-[calc(var(--u)*280)] overflow-hidden rounded-[calc(var(--u)*28)] bg-gradient-to-br from-[#f4efe6] to-[#e7dfd0]">
          ...
          <span className="absolute bottom-[calc(var(--u)*26)] left-[calc(var(--u)*30)] flex h-[calc(var(--u)*60)] items-center gap-[calc(var(--u)*12)] rounded-full bg-gold-dark px-[calc(var(--u)*30)] text-[calc(var(--u)*23)] text-white">
        ...
        <div className="no-scrollbar mt-[calc(var(--u)*30)] flex gap-[calc(var(--u)*14)] overflow-x-auto">
          {collection.chips.map((c) => (
            <button
              ...
              aria-pressed={chip === c}
              onClick={() => setChip(c)}
              className={cn("h-[calc(var(--u)*64)] shrink-0 rounded-full px-[calc(var(--u)*30)] text-[calc(var(--u)*24)]", chip === c ? "bg-gold-dark text-white" : "bg-pill/80")}
            >
              {c}
            </button>
          ))}
        ...
        <div className="mt-[calc(var(--u)*28)] flex gap-[calc(var(--u)*16)]">
          <label className="flex h-[calc(var(--u)*82)] flex-1 items-center gap-[calc(var(--u)*12)] rounded-full bg-pill/80 px-[calc(var(--u)*24)] text-[calc(var(--u)*24)]">
          ...
          <button
            ...
            className={cn("flex h-[calc(var(--u)*82)] items-center gap-[calc(var(--u)*12)] rounded-full px-[calc(var(--u)*28)] text-[calc(var(--u)*24)]", saleOnly ? "bg-gold-dark text-white" : "bg-pill/80")}
          >
          ...
          <div className="flex gap-[calc(var(--u)*10)]">
            {[false, true].map((isList) => (
              <button
                ...
                aria-pressed={list === isList}
                onClick={() => setList(isList)}
                className={cn("grid size-[calc(var(--u)*60)] place-items-center rounded-[calc(var(--u)*14)]", list === isList ? "bg-gold-dark text-white" : "bg-pill/80")}
              >
              ...
              return (
                <article key={p.id} data-card className={cn("overflow-hidden rounded-[calc(var(--u)*22)] bg-white shadow-[0_4px_24px_rgba(60,45,20,0.06)]", list && "flex")}>
                  <div className={cn("relative bg-card", list ? "h-[calc(var(--u)*260)] w-[calc(var(--u)*220)] shrink-0" : "h-[calc(var(--u)*300)]")}>
                    ...
                    {off > 0 && (
                      <span className="absolute left-[calc(var(--u)*10)] top-[calc(var(--u)*10)] rounded-full bg-white px-[calc(var(--u)*12)] py-[calc(var(--u)*6)] text-[calc(var(--u)*18)]">{off}% Off</span>
                    )}
