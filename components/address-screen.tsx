    ...
    <PhoneFrame>
      <main className="px-[calc(var(--u)*40)] pb-[calc(var(--u)*240)] pt-[calc(var(--u)*60)]">
        <header className="flex items-center justify-between">
          <button type="button" aria-label="Back" onClick={() => router.back()} className="grid size-[calc(var(--u)*92)] place-items-center rounded-full bg-pill/80">
            <ArrowLeft className="size-[calc(var(--u)*42)]" strokeWidth={1.6} />
          </button>
          <div className="text-center">
            <h1 className="font-display text-[calc(var(--u)*54)] leading-none">Our Store</h1>
        ...
        <section className="mt-[calc(var(--u)*30)] rounded-[calc(var(--u)*30)] bg-white p-[calc(var(--u)*34)] shadow-[0_4px_24px_rgba(60,45,20,0.05)]">
          ...
          <ul className="mt-[calc(var(--u)*24)] space-y-[calc(var(--u)*22)] text-[calc(var(--u)*27)]">
            <li className="flex gap-[calc(var(--u)*20)]"><MapPin className="mt-[calc(var(--u)*4)] size-[calc(var(--u)*36)] shrink-0 text-gold-dark" strokeWidth={1.6} />{shop.address}</li>
            <li className="flex gap-[calc(var(--u)*20)]"><Phone className="size-[calc(var(--u)*36)] shrink-0 text-gold-dark" strokeWidth={1.6} />{shop.phone}</li>
            <li className="flex gap-[calc(var(--u)*20)]"><Clock className="size-[calc(var(--u)*36)] shrink-0 text-gold-dark" strokeWidth={1.6} />{shop.hours}</li>
          </ul>
          <a
            href={`https://www.google.com/maps/dir/?api=1&destination=${q}`}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-[calc(var(--u)*30)] flex h-[calc(var(--u)*88)] items-center justify-center gap-[calc(var(--u)*14)] rounded-full bg-gold-dark text-[calc(var(--u)*27)] font-medium text-white"
          >
