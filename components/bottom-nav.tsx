...
export function BottomNav({ active, cartCount = 0, accent = FOREST }: { active: string; cartCount?: number; accent?: string }) {
  return (
          ...
          {items.map(({ href, label, Icon }) => {
            ...
            return (
              <Link
                ...
                className={cn(
                  "flex h-[calc(var(--u)*93)] w-[calc(var(--u)*186)] flex-col items-center justify-center gap-[calc(var(--u)*6)] rounded-[calc(var(--u)*46)] text-[calc(var(--u)*24)] transition-colors",
                  on ? "font-medium text-white" : "text-muted",
                )}
              ...
              >
                <span className="relative" data-nav-cart={href === "/cart" ? "" : undefined}>
                  ...
                  {href === "/cart" && cartCount > 0 && (
                    <span key={cartCount} className="absolute -right-[calc(var(--u)*18)] -top-[calc(var(--u)*14)] grid size-[calc(var(--u)*32)] place-items-center rounded-full text-[calc(var(--u)*19)] text-white animate-[cart-pop_0.45s_cubic-bezier(0.34,1.56,0.64,1)]" style={{ background: accent }}>
                      {cartCount}
                    </span>
                  )}
                </span>
