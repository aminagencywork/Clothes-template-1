...
export const primaryBtn =
  "flex h-[calc(var(--u)*100)] w-full items-center justify-center gap-[calc(var(--u)*20)] rounded-[calc(var(--u)*24)] bg-gold-dark text-[calc(var(--u)*30)] font-medium text-white transition-transform active:scale-[0.98] disabled:opacity-60";
export const card = "rounded-[calc(var(--u)*28)] bg-white/70 shadow-[0_2px_14px_rgba(60,45,20,0.05)]";
export const editLink = "text-[calc(var(--u)*25)] text-gold-dark underline underline-offset-4";
    ...
    <PhoneFrame>
      <main className="px-[calc(var(--u)*40)] pb-[calc(var(--u)*80)] pt-[calc(var(--u)*70)]">
        <header className="relative flex items-start justify-center">
          ...
          <Image src="/images/home-logo.jpg" alt="Vyntra – wear a brighter you" width={290} height={135} priority className="h-[calc(var(--u)*110)] w-auto mix-blend-multiply" />
        </header>
          ...
          {STEPS.map(({ label, Icon }, i) => {
            ...
            return (
              <li key={label} className="relative flex flex-1 flex-col items-center" aria-current={current ? "step" : undefined}>
                {i > 0 && (
                  <span className={cn("absolute right-1/2 top-[calc(var(--u)*33)] h-[calc(var(--u)*3)] w-full", n <= step ? "bg-gold-dark" : "bg-black/10")} />
                )}
                <span
                  className={cn(
                    "relative z-10 grid size-[calc(var(--u)*68)] place-items-center rounded-full",
                    done || current ? "bg-gold-dark text-white" : "bg-page text-muted",
                  )}
                >
...
export function Radio({ on }: { on: boolean }) {
  return (
    <span className={cn("grid size-[calc(var(--u)*40)] shrink-0 place-items-center rounded-full border-2", on ? "border-gold-dark" : "border-black/25")}>
      {on && <span className="size-[calc(var(--u)*20)] rounded-full bg-gold-dark" />}
    </span>
  );
}
